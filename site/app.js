...
// YouTube hosts the requested commercial songs. We only embed its official player.
...
function loadYouTube(id, label) {
  if (playing) pause();
  $("youtube-frame").src = `https://www.youtube-nocookie.com/embed/${id}?enablejsapi=1&rel=0`;
  $("youtube-frame").title = `${label} on YouTube`;
  $("youtube-open").href = `https://www.youtube.com/watch?v=${id}`;
  $("youtube-now").textContent = `${label} · Controls are inside the YouTube player`;
  $("youtube-display").hidden = false;
  $("youtube-empty").hidden = true;
  $("youtube-feedback").textContent = `${label} is ready. Press play in the YouTube player below.`;
  document.querySelectorAll(".youtube-pick").forEach(button => {
    const active = button.dataset.video === id;
    button.classList.toggle("selected", active);
    button.setAttribute("aria-pressed", String(active));
  });
}
document.querySelectorAll(".youtube-pick").forEach(button => button.addEventListener("click", () => {
  loadYouTube(button.dataset.video, button.dataset.label);
}));
$("youtube-form").addEventListener("submit", event => {
  event.preventDefault();
  const id = youtubeId($("youtube-url").value);
  if (!id) { $("youtube-feedback").textContent = "Paste a YouTube or YouTube Music video link to try again."; return; }
  loadYouTube(id, "Your YouTube pick");
});
$("youtube-remove").addEventListener("click", () => {
  $("youtube-frame").removeAttribute("src");
  $("youtube-display").hidden = true;
  $("youtube-empty").hidden = false;
  $("youtube-feedback").textContent = "Choose a song to load its YouTube player.";
  document.querySelectorAll(".youtube-pick").forEach(button => { button.classList.remove("selected"); button.setAttribute("aria-pressed", "false"); });
});
